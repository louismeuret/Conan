...
from docklaunch3 import monitor_docking_status
import psutil
import re
import os
import threading
...
async def monitor_docking_status(status_dict):
    while True:
        ...
        if dpg.does_item_exist("counter_text"):
            dpg.set_value("counter_text", f"{str(dict(status_dict))}")
...
def dockingtot(software2, nptsx, nptsy, nptsz, gridcenterx, gridcentery, gridcenterz, spacing, threads, nruns, pathdb, path_results, DEBUG_FLAG):
    cwd = os.getcwd()
...
def run(sender, data):
    ...
    with dpg.window(label="Example Window"):
        dpg.add_text("0", tag="counter_text")
