...
def fldcreation(ligandtype,dirmaps,path_ligand,gridcenterx, gridcentery,gridcenterz,cwd,nomreceptor,nptsx,nptsy,nptsz,spacing):
    ...
    if float(gridcenterx) < 0:
        nelementsx = str(int(nptsx)-1)
    else:
        nelementsx = str(int(nptsx))
    if float(gridcentery) < 0:
        nelementsy = str(int(nptsy)-1)
    else:
        nelementsy = str(int(nptsy))
    if float(gridcenterz) < 0:
        nelementsz = str(int(nptsz)-1)
    else:
        nelementsz = str(int(nptsz))

    fldcontenu[6] = "#SPACING "+spacing+"\n"
    fldcontenu[7] = "#NELEMENTS "+nelementsx+" "+nelementsy+" "+nelementsz+"\n"
    fldcontenu[8] = "#CENTER "+gridcenterx+" "+gridcentery+" "+gridcenterz+"\n"
    fldcontenu[9] = "#MACROMOLECULE "+pathreceptor+"\n"
    fldcontenu[13] = "dim1="+str(int(nelementsx)+1)+"\n"
    fldcontenu[14] = "dim2="+str(int(nelementsy)+1)+"\n"
    fldcontenu[15] = "dim3="+str(int(nelementsz)+1)+"\n"
