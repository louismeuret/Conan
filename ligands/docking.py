...
def docking(software:str,nptsx:str,nptsy:str,nptsz:str,gridcenterx:str,gridcentery:str,gridcenterz:str,spacing:str,threads:str,nruns:str,pathdb:str,DEBUG_FLAG:bool,listofpdbqt:list,dossiertemps:str,listofreceptors:list,dirmaps:list,cwd:str,pathsoftware:str,
ligandnumber):
    #print(FAIL)
    ...
    for receptor in range(len(listofreceptors)):
        ...
        if software == "GPU" or software == "AD4":
            ligandtype = dpf.dpfcreation(
                pathligand, path_receptor, dpflocation, dirmap, cwd, nomreceptor,software)
        #print("ligand type: ")
        # print(ligandtype)
            if software == "GPU":
                fld.fldcreation(ligandtype, dirmap, path_ligand, gridcenterx,
                        gridcentery, gridcenterz, cwd, nomreceptor, nptsx, nptsy, nptsz,spacing)
            elif software == "AD4":
                fld2.fldcreation(ligandtype, dirmap, path_ligand, gridcenterx,
                        gridcentery, gridcenterz, cwd, nomreceptor, nptsx, nptsy, nptsz,spacing)
            ...
            if (os.path.isfile(pathbest)) == False:
                ...
                if DEBUG_FLAG == True:
                    if software == "GPU":
                        ...
                        os.system(runautodock)
                    if software == "AD4":
                        runautodock = pathsoftware + " -p "+dpflocation+" -l "+pathligand+"log.txt"
                        os.system(runautodock)
